...
#include "ISXEQAutoCamp.h"

// The mandatory pre-setup function.  Our name is "ISXEQAutoCamp", and the class is ISXEQAutoCamp.
...
// our instanced class.
ISXPreSetup("ISXEQAutoCamp",ISXEQAutoCamp);
...
unsigned int ISXEQAutoCampXML=0;
...
bool ISXEQAutoCamp::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQAutoCamp Loaded");
return true;
}

// shutdown sequence
void ISXEQAutoCamp::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQAutoCampXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQAutoCampXML);
...
void ISXEQAutoCamp::ConnectServices()
{
...
void ISXEQAutoCamp::RegisterCommands()
{
    // add any commands
...
void ISXEQAutoCamp::RegisterAliases()
{
    // add any aliases
}

void ISXEQAutoCamp::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQAutoCamp::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAutoCamp::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQAutoCamp::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQAutoCamp::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAutoCamp::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQAutoCampService)
    //        pISInterface->ShutdownService(this,hISXEQAutoCampService);
}

void ISXEQAutoCamp::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQAutoCampXML
    ISXEQAutoCampXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQAutoCampXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQAutoCampXML,"General"))
    {
